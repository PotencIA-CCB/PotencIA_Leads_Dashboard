  ...
  useEffect(() => {
    ...
    async function fetchMetricas() {
      ...
      const consultoriasQuery = supabase
        .from('consultorias')
        .select('id, fecha, status, servicio, duracion_minutos, id_consultor, id_lead, categoria_caso_uso, categoria_caso, nivel_potencia, hora_inicio, modalidad, leads!inner(nombre, apellidos, city, company_role_level, origen, sector, company_role_area, nit), consultores(nombre)')
