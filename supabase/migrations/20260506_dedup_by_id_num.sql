...
-- Migration: cross-channel dedup by cédula (id_num).
--
-- Problem: a customer can submit the landing form with one email and book via
-- Microsoft Bookings with a different email. Email-only matching creates duplicates.
--
-- Solution: add id_num (cédula) as a strong matching signal, since it's unique per person
-- and stable across channels. Match priority becomes:
--   1. booking_customer_id (Microsoft stable ID, when present)
--   2. email (canonical)
--   3. id_num (cédula) — NEW
--   4. phone_window (conservative, bookings only)
-- Plus a one-shot retroactive dedup of leads that already share an id_num.

begin;

-- ---------------------------------------------------------------------------
-- 1) Index on id_num for fast lookups
-- ---------------------------------------------------------------------------
create index if not exists leads_id_num_idx on public.leads (id_num) where id_num is not null;

-- ---------------------------------------------------------------------------
-- 2) merge_prelead_into_leads: add id_num matching after email
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_lead_id uuid;
begin
  if v_email = '' and v_id_num is null then
    return null;
  end if;

  -- 1) Match by email (canonical or booking_email)
  if v_email <> '' then
  ...
  -- 2) Match by cédula (cross-channel: same person, different email)
  ...
  if v_lead_id is not null then
    -- Enrich-only: COALESCE preserves any existing value
  ...
  -- No match: create new landing lead
...
-- ---------------------------------------------------------------------------
-- 3) merge_or_create_lead: extend signature with p_id_num and p_nit
-- ---------------------------------------------------------------------------
drop function if exists public.merge_or_create_lead(text, text, text, text, text, uuid, text, int);
...
begin
  ...
  -- 2) Match by email (booking_email or canonical)
  select l.id into v_lead_id
  from public.leads l
  ...
  -- 3) Match by cédula (cross-channel: form filled with one email, booking with another)
  ...
  -- 4) Conservative phone-window match (bookings only)
  ...
  -- 5) Brand-new lead from Bookings
  insert into public.leads (
...
-- ---------------------------------------------------------------------------
-- 4) Retroactive dedup: merge leads sharing id_num that the old logic missed
-- ---------------------------------------------------------------------------
do $$
declare
  r record;
...
begin
  for r in
    select array_agg(id order by
      -- Pick the lead with the strongest booking signals as primary
      case
    ...
    from public.leads
    where id_num is not null and trim(id_num) <> ''
    group by id_num
    having count(*) > 1
  loop
    v_primary := r.ids[1];
    ...
    for i in 2..array_length(r.ids, 1) loop
      ...
      -- Enrich primary with any non-null fields from duplicate
      ...
      set
        ...
        phone_normalized      = coalesce(p.phone_normalized,            d.phone_normalized),
        ...
        -- Status: keep the more advanced status if duplicate has one
        status = case
      ...
      -- Reassign sesiones to the primary lead
      update public.sesiones set id_lead = v_primary where id_lead = v_dup;

      -- Drop the duplicate
      delete from public.leads where id = v_dup;

      raise notice 'Dedup: merged lead % into % by shared id_num', v_dup, v_primary;
    end loop;
  end loop;
end$$;
